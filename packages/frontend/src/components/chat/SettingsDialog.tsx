...
interface SettingsDialogProps {
    ...
    previewBuilds: UsePreviewBuildsResult;
}
...
const SettingsDialog: React.FC<SettingsDialogProps> = ({ open, onOpenChange, previewBuilds }) => {
    const {
        instructions,
        activeInstructionId,
        activeInstruction,
    ...
    }, [activeTab, markAllSeen, open, toast]);
                ...
                <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as SettingsTab)}>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Textarea
                                                    ...
                                                    onBlur={persistChanges}
                                                    rows={5}
                                                    className="min-h-[3.75rem] resize-none font-mono text-sm"
                                                />
                                            </div>
                                        </div>
                        ...
                        <div className="space-y-6">
                            <RadioGroup value={mode} onValueChange={(value) => setMode(value as ConversationContextMode)}>
                                ...
                                <div
                                    className={cn(
                                        'rounded-md border p-4 transition-colors',
                                        mode === 'intelligent' ? 'border-primary bg-primary/5' : 'border-muted'
                                    )}
                                >
                                    <div className="flex items-start gap-3">
                                        <RadioGroupItem value="intelligent" id="context-intelligent" />
                                        <Label htmlFor="context-intelligent" className="flex-1 cursor-pointer space-y-1">
                                            <p className="text-sm font-medium">Intelligent (Hierarchical Summaries)</p>
                                            <p className="text-xs text-muted-foreground">
                                                Generate tiered conversation summaries to keep long-running threads coherent and affordable.
                                            </p>
                                        </Label>
                                    </div>
                                    {mode === 'intelligent' && (
                                        <div className="mt-4 space-y-4 rounded-md border bg-background/80 p-4">
                                            ...
                                            <div className="space-y-2">
                                                <Label htmlFor="forced-recent-count" className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                                                    Forced Recent Messages: {forcedRecentMessageCount}
                                                </Label>
                                                <Slider
                                                    id="forced-recent-count"
                                                    value={[forcedRecentMessageCount]}
                                                    min={0}
                                                    max={6}
                                                    step={1}
                                                    onValueChange={(values) => setForcedRecentMessageCount(values[0] ?? 0)}
                                                />
                                                <p className="text-xs text-muted-foreground">
                                                    Number of most recent messages (0-6) to always include, regardless of summarization. Set to 0 to rely entirely on summaries and today's messages.
                                                </p>
                                            </div>
                                        </div>
                                    )}
                    ...
                    <TabsContent value="builds" className="mt-6">
                        <PreviewBuildsPanel
                            ...
                            loading={previewBuildLoading}
                            error={previewBuildError}
                            committingPrNumbers={committingPrNumbers}
                            commitBuild={commitBuild}
                            refresh={refreshPreviewBuilds}
                        />
                    </TabsContent>
                </Tabs>
