...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, isStreaming, onSave, branchInfo, onActivate, isActiveSnapshot, isHistoricalView }) => {
    ...
    const containerClasses = cn('flex max-w-full min-w-0 box-border px-2 md:px-4', message.role === 'user' ? 'justify-start' : 'justify-start');
    const bubbleClasses = cn(
        'relative max-w-full min-w-0 box-border rounded-lg px-2 py-3 pb-6 transition-shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 break-words overflow-wrap-anywhere',
