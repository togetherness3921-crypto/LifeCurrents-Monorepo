...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, isStreaming, onSave, branchInfo, onActivate, isActiveSnapshot, isHistoricalView }) => {
    ...
    const containerClasses = cn('flex w-full overflow-hidden', message.role === 'user' ? 'justify-start' : 'justify-start');
    const bubbleClasses = cn(
        'relative w-full rounded-lg px-4 py-3 transition-shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 break-words overflow-wrap-anywhere box-border',
