...
/**
 * OBJECTIVE 3: Compress stale tool calls for LLM context efficiency
 *
 * Generates a programmatic summary for a tool call by inspecting its arguments.
 * This is NOT generated by the LLM, but by parsing the tool call structure.
 */
const generateToolCallSummary = (toolCall: { name: string; arguments: string; response?: string; status: string }): string => {
    try {
        const args = typeof toolCall.arguments === 'string' ? JSON.parse(toolCall.arguments) : toolCall.arguments;

        if (toolCall.name === 'patch_graph_document' && args.patches) {
            ...
            if (Array.isArray(patches)) {
                ...
                if (removeCount > 0) parts.push(`${removeCount} node${removeCount !== 1 ? 's' : ''} removed`);
                if (replaceCount > 0) parts.push(`${replaceCount} update${replaceCount !== 1 ? 's' : ''} made`);

                return JSON.stringify({
                    status: toolCall.status,
                    outcome: `Graph patched successfully. ${parts.join(', ')}.`
                });
            }
        }
        ...
        return JSON.stringify({
            status: toolCall.status,
            outcome: `${toolCall.name} completed successfully.`
        });
    } catch (error) {
        // Fallback if parsing fails
        return JSON.stringify({
            status: toolCall.status,
            outcome: `${toolCall.name} executed.`
        });
    }
};
...
export const prepareIntelligentContext = async (
    ...
    if (!options.branchHeadMessageId) {
        ...
        return {
            ...
            recentMessages: compressStaleToolCalls(recentMessages),
        };
    }
